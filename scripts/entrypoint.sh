...
function create_system_user_if_missing() {
    ...
    # HOME directory to /opt/airflow
    if ! whoami &> /dev/null; then
      if [[ -w /etc/passwd ]]; then
        echo "${USER_NAME:-airflow}:x:$(id -u):0:${USER_NAME:-airflow} user:${AIRFLOW_HOME}:/sbin/nologin" \
            >> /etc/passwd
      fi
...
function check_uid_gid() {
    ...
    if [[ $(id -u) == "1001" ]]; then
        ...
        >&2 echo "         even if you use 'airflow' user (UID=1001)"
        >&2 echo
